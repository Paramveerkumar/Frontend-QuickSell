...
const KanbanBoardColumnStatus = ({ initialTasks, users, ordering }) => {
    ...
    // Sorting tasks within each column based on ordering (priority or title)
    const sortTasks = (tasks) => {
        if (ordering === 'priority') {
            return tasks.sort((a, b) => b.priority - a.priority); // Sort by priority (descending)
        } else if (ordering === 'title') {
            return tasks.sort((a, b) => a.title.localeCompare(b.title)); // Sort by title (alphabetical)
        }
        return tasks;
    };
                ...
                {columns.map(column => (
                    ...
                    >
                        <div className="kanban-column-header">
                            <h2>{column} <span className="task-count">{tasks.filter(task => task.status === column).length}</span></h2>
                        ...
                        {sortTasks(tasks.filter(task => task.status === column))
                            .map(task => (
                                <div
                                    key={task.id}
                                    className="task-card"
                                ...
                                >
                                    ...
                                    {
                                        task.tag.map(e => e.toLowerCase()).includes("feature request") && <div className="task-footer">
                                            <span className="task-tag"><span className='task-tag-dot'></span> Feature Request</span>
                                        </div>
                                    }
                                </div>
                            ))
...
export default KanbanBoardColumnStatus;