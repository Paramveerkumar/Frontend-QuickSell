...
const KanbanColoumnUngency = ({ initialTasks, users }) => {
    const [tasks, setTasks] = useState(initialTasks);

    const columns = [
        { name: 'No Priority', level: 0 },
        { name: 'Low Priority', level: 1 },
        { name: 'Medium Priority', level: 2 },
        { name: 'High Priority', level: 3 },
        { name: 'Urgent Priority', level: 4 },
    ];
    ...
    // Change task's priority when dropping into a different column
    ...
    // Sort tasks by title within each priority column
            ...
            <div className="kanban-columns">
                {columns.map(column => (
                    <div
                        ...
                        onDrop={(e) => handleDrop(e, column.level)}
                    >
                        <div className="kanban-column-header">
                            <h2>{column.name} <span className="task-count">{tasks.filter(task => task.priority === column.level).length}</span></h2>
                        ...
                        {sortTasksByTitle(tasks.filter(task => task.priority === column.level))
                            .map(task => (
                                <div
                                    key={task.id}
                                    className="task-card"
...
export default KanbanColoumnUngency;